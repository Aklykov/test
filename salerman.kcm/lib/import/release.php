...
use \Salerman\Kcm\ReleaseTable as ReleaseTable;
...
use \Salerman\Kcm\Exception\ReleaseImportException as ReleaseImportException;

/**
 * Класс-стратегия реализует логику импорта "Выпуск"
 *
 * Class Release
 * @package Salerman\Kcm\Import
 */
class Release implements ImportStrategy
{
	public $arListPathPdf;

	function getSubdirNew()
	{
		return KCM__PATH_TO_XML_BP2.self::SUBDIR_NEW;
	}

	function getSubdirError()
	{
		return KCM__PATH_TO_XML_BP2.self::SUBDIR_ERROR;
	}

	function getSubdirArchive()
	{
		return KCM__PATH_TO_XML_BP2.self::SUBDIR_ARCHIVE;
	}
	...
	{
		return KCM__PATH_TO_PDF_BP2.self::SUBDIR_NEW.$filename;
	}
	...
	{
		return KCM__PATH_TO_PDF_BP2.self::SUBDIR_ARCHIVE.$FC_CONTRACT.'/'.$NUM.'/'.$filename;
	}
	...
	{
		return KCM__PATH_TO_PDF_BP2.self::SUBDIR_ERROR.$filename;
	}
	...
	function checkXml($objXml)
	{
		...
		if( empty($arXml['R_FG']['FC_CONTRACT']) && empty($arXml['R_FG']['FC_CONTRACT_ADD']) ) {
			throw new ReleaseImportException('Не заполнено обязательное поле FC_CONTRACT или FC_CONTRACT_ADD (номер договора)');
		}
		if( empty($arXml['R_FG']['FC_BP']) ) {
			throw new ReleaseImportException('Не заполнено обязательное поле FC_BP (номер контрагента)');
		}
		if( empty($arXml['R_FG']['FK_DOC_NUM']) ) {
			throw new ReleaseImportException('Не заполнено обязательное поле FK_DOC_NUM (Номер поставки)');
		}
		return true;
	}
	...
	function getJson($objXml)
	{
		...
		// Заполняем table
		if( !empty($arXml['R_FG']['T_BATCH']['R_BATCH'][0]) ) {
			$arR_BATCH = $arXml['R_FG']['T_BATCH']['R_BATCH'];
		} else {
			$arR_BATCH = $arXml['R_FG']['T_BATCH'];
		}
...
//					'weight' => $FN_MASS, // Лигатурная масса
	...
	function getListPathPdf($objXml, $FC_CONTRACT, $FC_BP, $NUM)
	{
		...
		$pdf = $arXml['R_FG']['FC_DOCS'];
		$arFileMap = FileMap::getListBP2();
		foreach( $arFileMap as $objFM ) {
			...
			foreach($arType as $type) {
				...
				$arListPathPdf[] = array(
					'name' => $objFM->name,
					'href' => '/file.php?entity=release&doc_id=' . $FC_CONTRACT . '&num_id=' . $NUM . '&fc_bp=' . $FC_BP . '&filename=' . $filename,
					'pathFrom' => $pathFrom,
					'pathTo' => $pathTo,
					'pathError' => $pathError
				);
	...
	{
		$arXml = object_to_array($objXml);
		$FC_BP = $arXml['R_FG']['FC_BP'];
		$FC_CONTRACT = $arXml['R_FG']['FC_CONTRACT'];
		$FC_CONTRACT_ADD = $arXml['R_FG']['FC_CONTRACT_ADD'];
		$FK_DOC_NUM = $arXml['R_FG']['FK_DOC_NUM'];
		$FD_DOC_DATE = $arXml['R_FG']['FD_DOC_DATE'];
		$IS_STORNO = $arXml['R_FG']['FB_IS_STORNO'] == 'false' ? 0 : 1;
		...
		// Получаем список металов (сумму всех слитков)
		...
		if( $arElement = ReleaseTable::getByInfo($FC_CONTRACT, $FC_BP, $FK_DOC_NUM) ) {
			$objResult = ReleaseTable::update($arElement['ID'], $data);
			if( $objResult->isSuccess() === false ) {
				$arErrors = $objResult->getErrorMessages();
				throw new ReleaseImportException($arErrors[0]);
			}
		} else {
			$objResult = ReleaseTable::add($data);
			if( $objResult->isSuccess() === false ) {
				$arErrors = $objResult->getErrorMessages();
				throw new ReleaseImportException($arErrors[0]);
			}
