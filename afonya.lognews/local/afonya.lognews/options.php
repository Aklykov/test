...
$arAllOptions = array_filter([
	[
		'UF_IBLOCK_ID_NEWS',
		Loc::getMessage('UF_IBLOCK_ID_NEWS'),
		$arDefValues['UF_IBLOCK_ID_NEWS'],
		['selectbox', $arIblocks]
	]
]);
...
if (($_SERVER['REQUEST_METHOD'] == 'POST') && (($_POST['Update'] || $_POST['Apply'] || $_POST['RestoreDefaults']) > 0) && check_bitrix_sessid()) {
	if ($_POST['RestoreDefaults'] <> '') {
		foreach($arDefValues as $key=>$value) {
			COption::RemoveOption('afonya.lognews', $key);
		}
		...
		foreach($arAllOptions as $arOption) {
			...
			COption::SetOptionString('afonya.lognews', $name, $val, $arOption[1]);
		}
	}
...
HTML;
		...
		if (!is_array($arOption)) {
			...
			$val = COption::GetOptionString('afonya.lognews', $arOption[0], $arOption[2]);
		}
